...
const PRIVACY_DOC_ID = (import.meta.env.VITE_PRIVACY_DOC_ID as string | undefined) ?? "1Ajm7WfQGrw1cI9Fx7q9MhQU1w1NLIJxLzkH9JPS3ii4";
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setError("");
        const nextHtml = await getDocPageHtml(PRIVACY_DOC_ID ?? "");
        if (!active) return;
        setHtml(nextHtml);
      } catch (err) {
        if (!active) return;
