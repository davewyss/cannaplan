...
const SOBRE_APP_DOC_ID = (import.meta.env.VITE_SOBRE_APP_DOC_ID as string | undefined) ?? "1UZLcaCsqZWndI4zIyzvdzfwaU8OPyh2RoFbKydRfu2E";
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setError("");
        const nextHtml = await getDocPageHtml(SOBRE_APP_DOC_ID ?? "");
        if (!active) return;
        setHtml(nextHtml);
      } catch (err) {
        if (!active) return;
