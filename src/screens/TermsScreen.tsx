...
const TERMS_DOC_ID = (import.meta.env.VITE_TERMS_DOC_ID as string | undefined) ?? "1iiwVvJs3aOm9TLBAPZmI-7V5zp8AC4UlJVGmSVV2EbE";
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setError("");
        const nextHtml = await getDocPageHtml(TERMS_DOC_ID ?? "");
        if (!active) return;
        setHtml(nextHtml);
      } catch (err) {
        if (!active) return;
