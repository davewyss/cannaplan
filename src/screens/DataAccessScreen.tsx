...
const DATA_ACCESS_DOC_ID = (import.meta.env.VITE_DATA_ACCESS_DOC_ID as string | undefined) ?? "165oZvuqMb2-WEen_p4AeEpJGMiip8CJSW4p80Xk9Vd8";
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setError("");
        const nextHtml = await getDocPageHtml(DATA_ACCESS_DOC_ID ?? "");
        if (!active) return;
        setHtml(nextHtml);
      } catch (err) {
        if (!active) return;
